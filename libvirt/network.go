...
func getIPsFromResource(d *schema.ResourceData) ([]libvirtxml.NetworkIP, error) {
	...
	for num, addressI := range addresses.([]interface{}) {
		...
		if dhcpEnabled {
			_, ipNet, err := net.ParseCIDR(addressI.(string))
			if err != nil {
				return nil, fmt.Errorf("Error parsing addresses definition '%s': %s", addressI.(string), err)
			}
			if err := setStaticDHCPHosts(ipNet, d, dhcp); err != nil {
				return nil, err
			}
			dni.DHCP = dhcp
		} else {
